// Inspired from https://www.viralsciencecreativity.com/post/arduino-hourglass-with-matrix-display-and-mpu6050
// Smart Digital Hourglass
// Using Arduino UNO, 2 8x8 LED Matrix, ADXL335 3 axis accelerometer
...
// Matrix
#define PIN_DATAIN 12 //16
#define PIN_CLK 10 //14
#define PIN_LOAD 11 //15
...
void alarm() {
  //Serial.print("drop_time: ");
  //Serial.println(drop_time);
  lc.clearDisplay(0);  
      lc.clearDisplay(1);

  // Only 1 buzzer-sequence after boot/reset
    for (int k=0; k<1; k++) {
      for (int i=0; i<3; i++) {
      delay(100);
      tone(PIN_BUZZER, 1000, 500);
      for (byte m=0; m<8; m++) {
    lc.setXY(MATRIX_B, 0, m, true);
    lc.setXY(MATRIX_B, m, 0, true);
    }
    for (byte m=7; m>0; m--) {
    lc.setXY(MATRIX_B, 7, m, true);
    lc.setXY(MATRIX_B, m, 7, true);
    }
    for (byte m=0; m<8; m++) {
      lc.setXY(MATRIX_A, 0, m, true);
      lc.setXY(MATRIX_A, m, 0, true);
    }
    for (byte m=7; m>0; m--) {
    lc.setXY(MATRIX_A, 7, m, true);
    lc.setXY(MATRIX_A, m, 7, true);
    }   
